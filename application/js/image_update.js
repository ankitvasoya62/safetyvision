...
var ImageCrop = function() {
    var me = this;
    me.verify = {title: true, image: true};
    me.x = 160;
    me.y = 90;
    me.x2 = 640;
    me.y2 = 450;
    ...
    this.init_image = function(){
        var img = new Image();
        img.src = $("#crop-pane").attr("data_src");
        img.onload = function() {
            $("#crop-pane").html('');
            $("#preview-pane").css("display", "block");
            $("#crop-pane").html('<img src="' + $("#crop-pane").attr("data_src") + '" id="target" />');
            $(".preview-container").html('<img src="' + $("#crop-pane").attr("data_src") + '" id="preview" />');
            me.crop();
        };
    };
    ...
    this.submit = function() {
        $("#submit-button").click(function() {
            ...
            $id = $("#crop_id").val();
            $.post("/screen/spots/handleImage", $.param({image: {token: $token, x: $x, y: $y, width: $width, height: $height, id:$id}, title: $title}), function(data) {
                if (data.status) {
                    window.location.href = '/screen/spots/update/id/'+$("#crop-pane").attr("data_id")+'/step/1/token/' + data.token;
                }
            }, "json");
        });
    };
