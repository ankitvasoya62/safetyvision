...
var Video = function () {
            ...
            if (me.checkType()) {
                ...
                $("#uploadForm").ajaxSubmit({
                    ...
                    beforeSubmit: function () {
                        if(me.type == 'mp4'){
                            $("#progress .progress_speed").css('width', '100%');
    ...
    this.submit = function () {
        ...
        $("#submit-button").click(function () {
            $token = $("#token").val(), $spot_id = $("#spot-form").attr("data_id");
            window.location.href = '/screen/spots/update/step/2/id/' + $spot_id + '/token/' + $token;
        });
    };
    ...
    this.save = function () {
        $("#spot-submit-button").click(function () {
            if (me.verifySpot()) {
                ...
                $.post('/screen/spots/save', {spot: {token: $token, owner: $owner, start_date: $start_date, stop_date: $stop_date, screen_id: $screen_ids,start_hh:$start_hh,stop_hh:$stop_hh},spot_id:$("#spot-form").attr("data_id")}, function (data) {
                    if (data.status) {
